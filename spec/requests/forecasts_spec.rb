...
RSpec.describe "Forecasts", type: :request do
  describe "GET /forecasts" do
    ...
    it "returns a successful response" do
      get forecasts_path, params: { search_query: { latitude: latitude, longitude: longitude, formatted_address: 'Foo, Bar' } }

      aggregate_failures do
        expect(response).to have_http_status(:success)
        expect(response.body).to include("Current Conditions")
        expect(response.body).to include("#{condition.temperature}#{condition.temperature_unit}")
        expect(response.body).to include("#{daily_forecasts.size} Day Forecast")
        expect(response.body).to include("#{daily_forecasts.first.temperature}#{daily_forecasts.first.temperature_unit}")
      end
    end
  end
end
